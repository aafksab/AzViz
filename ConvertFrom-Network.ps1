function ConvertFrom-Network {
    ...
    param (
        ...
        [int] $CategoryDepth = 1,
        [string[]] $ExcludeTypes,
        [ValidateSet('virtualNetworks','subnets','networkSecurityGroups')]
        [string[]] $NetworkTypes
    )
    ...
    begin {
        ...
        # Default excluded network objects (these are excluded unless the caller explicitly requests them via -NetworkTypes)
        $DefaultExcluded = @(
        ...
        # Start with the default exclusions, allow user-specified additional exclusions via -ExcludeTypes
        $Excluded_NetworkObjects = @($DefaultExcluded)

        if ($ExcludeTypes) {
            $Excluded_NetworkObjects += $ExcludeTypes
        }

        # If the caller provided NetworkTypes to include, remove the corresponding patterns from the exclusion list
        if ($NetworkTypes) {
            foreach ($nt in $NetworkTypes) {
                switch ($nt) {
                    'virtualNetworks' {
                        $Excluded_NetworkObjects = $Excluded_NetworkObjects | Where-Object { $_ -ne "*Microsoft.Network/virtualNetworks*" }
                    }
                    'subnets' {
                        $Excluded_NetworkObjects = $Excluded_NetworkObjects | Where-Object { $_ -ne "*Microsoft.Network/virtualNetworks/subnets*" }
                    }
                    'networkSecurityGroups' {
                        $Excluded_NetworkObjects = $Excluded_NetworkObjects | Where-Object { $_ -ne "*Microsoft.Network/networkSecurityGroups*" }
                    }
                }
            }
        }
                
        # Build the scriptblock used to filter the final Resources: exclude patterns remaining in $Excluded_NetworkObjects
